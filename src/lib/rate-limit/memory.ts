...
function defaults(opt?: Partial<RateLimitOptions>): Required<RateLimitOptions> {
    return {
        limit: opt?.limit ?? 60,          // 60 req / window
        windowMs: opt?.windowMs ?? 60_000,// 1 phút
        burst: opt?.burst ?? opt?.limit ?? 60
    }
}

/** Token Bucket in-memory: mượt hơn fixed-window, có burst. */
export class InMemoryTokenBucket implements RateLimiter {
    ...
    async consume(key: string, o?: Partial<RateLimitOptions>): Promise<RateLimitResult> {
        ...
        const ratePerMs = cfg.limit / cfg.windowMs // số token refill mỗi ms
        ...
        b.tokens -= 1
        // thời điểm đầy bình (không bắt buộc), ở header ta ưu tiên “token kế tiếp sẵn sàng”
