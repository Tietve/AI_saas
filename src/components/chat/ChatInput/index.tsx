...
export const ChatInput: React.FC<ChatInputProps> = ({
    ...
    return (
        <div className="chat-input-container border-t border-transparent">
            <form onSubmit={handleSubmit}>
                <div className="px-4 py-3">
                    <div className="max-w-3xl mx-auto">
                        <div className="relative">
                            <div className={`chat-input-shell flex flex-col gap-2 ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
                                <input
                                    ref={fileInputRef}
                                    type="file"
                                    multiple
                                ...
                                <div className="flex items-end gap-2">
                                    ...
                                    <textarea
                                        ...
                                        rows={rows}
                                        className="chat-textarea flex-1 resize-none outline-none text-sm disabled:cursor-not-allowed"
                                        style={{
                                            lineHeight: '24px',
                                ...
                                <div className="flex flex-wrap gap-2 mt-3">
                                    {attachments.map(att => {
                                        ...
                                        return (
                                            <div
                                                key={att.id}
                                                className="chat-attachment-preview relative group overflow-hidden"
                                            >
                                                {isImage ? (
                                                    <div
                                                        className="w-32 h-24 flex items-center justify-center overflow-hidden"
                                                        style={{ background: 'var(--chat-surface-soft)' }}
                                                    >
                                                        <img
                                                            src={att.url}
                                                            alt={name}
                                                    ...
                                                    <div
                                                        className="w-32 h-24 flex flex-col items-center justify-center gap-2 text-sm"
                                                    ...
                                                    >
                                                        ...
                                                        {size && (
                                                            <span className="text-[10px]" style={{ color: 'var(--chat-muted)' }}>{formatSize(size)}</span>
                                                        )}
                                                    </div>
                                                )}
                                                <button
                                                ...
                                                <div className="attachment-name absolute inset-x-0 bottom-0 px-2 py-1 truncate">
                                                    {name}
                                                </div>
                                            </div>
                                        )
                        ...
                        <div className="chat-input-footer flex items-center justify-between mt-2 px-1">
                            <span>
                                {disabled ? (
