...
export class BillingClientService {
  ...
  private readonly REQUEST_TIMEOUT = 1000; // 1 second timeout

  constructor() {
    // FIX: Đổi port từ 3003 → 3004 (billing service port)
    this.billingServiceUrl = process.env.BILLING_SERVICE_URL || 'http://localhost:3004';
  }
  ...
  async checkQuota(userId: string, sessionCookie: string): Promise<UsageInfo> {
    try {
      const response = await axios.get(`${this.billingServiceUrl}/api/usage`, {
        ...
        timeout: this.REQUEST_TIMEOUT, // Add timeout to prevent hanging
      });
