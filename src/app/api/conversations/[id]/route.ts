...
export async function GET(_: NextRequest, ctx: { params: Promise<{ id: string }> }) {
    try {
        ...
        const convo = await prisma.conversation.findFirst({
            where: { id, userId },
            select: { id: true, title: true, systemPrompt: true, model: true, createdAt: true, updatedAt: true },
        })
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (title === undefined && systemPrompt === undefined && model === undefined) {
        ...
        const updated = await prisma.conversation.update({
            ...
            select: { id: true, title: true, systemPrompt: true, model: true, updatedAt: true },
        })
